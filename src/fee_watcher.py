...
class FeeWatcher:
    ...
    Sinyal ke executor dikirim via shared file / HTTP POST.
    """
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    CANTEX_QUOTE_URL, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=10)
                ) as resp:
                    if resp.status == 401:
                        if not self._cookie_expired_notified:
                            self._cookie_expired_notified = True
                            log.warning("Cookie Cantex expired (401)")
                        return None
                    if resp.status != 200:
