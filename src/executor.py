...
class AccountExecutor:
        ...
        try:
            ...
            while success_count < DAILY_TX_LIMIT:
                ...
                if is_roundtrip_start and self.fee_gate_fn is not None:
                    # Pass success_count dan attempt — skip cek hanya di attempt=1 pair=1
                    ok, current_fee = await self.fee_gate_fn(success_count, attempt)
                    if not ok:
                        self.log.info(
    ...
    async def _resolve_swap_direction(self, info: any, success_count: int) -> tuple[str, str]:
        ...
        try:
            ...
            # Normalize: cari USDCx dan cBTC/CBTC case-insensitive
            def _get_bal(sym: str) -> Decimal:
                if sym in balances:
                    return balances[sym]
                sym_lower = sym.lower()
                for k, v in balances.items():
                    if k.lower() == sym_lower:
                        return v
                return Decimal("0")
            usdc_bal = _get_bal("USDCx")
            cbtc_bal = max(_get_bal("cBTC"), _get_bal("CBTC"))
    ...
    async def _compute_amount(
        ...
        *,
        ...
        try:
            ...
            # Normalize lookup: cBTC dan CBTC diperlakukan sama
            def _get_balance(sym: str) -> Decimal:
                if sym in balances:
                    return balances[sym]
                ...
                for k, v in balances.items():
                    if k.lower() == sym_lower:
                        return v
                return Decimal("0")
            balance = _get_balance(sell_symbol)
    ...
    async def _execute_swap(self, *, sdk, tx_index, sell_sym, buy_sym, sell_amount, instruments) -> TxResult:
        ...
        try:
            # Normalize instrument lookup: cBTC dan CBTC dianggap sama
            def _get_instr(sym: str):
                ...
                for k, v in instruments.items():
                    if k.lower() == sym_lower:
                        return v
                return None
            sell_instr = _get_instr(sell_sym)
            buy_instr = _get_instr(buy_sym)
