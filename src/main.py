...
from .signal_broker import ExecutorServer, SignalClient, SignalServer, fetch_all_executor_progress
...
class CantexOrchestrator:
    ...
    async def _run_fee_watcher_mode(self) -> None:
        ...
        # Warmup pre-auth untuk akun lokal Railway 1
    ...
    async def _daily_summary_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                text = await _build_daily_summary(yesterday)
                if self._panel:
    ...
    async def _on_config_updated(self, what: str) -> None:
        ...
        if what in ("fee_cookie", "max_fee") and self._fee_watcher_instance:
            cfg = FeeWatcherStore.load()
            self._fee_watcher_instance.config = cfg
            log.info("Fee watcher config diupdate: max_fee=%.6f CC", cfg.max_fee_cc)
    ...
    async def _run_executor_mode(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            async def _fee_gate_remote(success_count: int, attempt: int = 1) -> tuple[bool, float | None]:
                # Skip cek hanya di attempt pertama pair pertama
    ...
    async def _run_accounts_parallel(self, *, fee_cc: float, today: str) -> list[DayResult]:
        ...
        # Cache max_fee sekali — tidak perlu baca disk tiap fee gate call
        _max_fee_cc = FeeWatcherStore.load().max_fee_cc

        async def _fee_gate(success_count: int, attempt: int = 1) -> tuple[bool, float | None]:
            # Hanya skip cek fee di attempt pertama pair pertama (percaya sinyal awal)
            # Retry setelah timeout HARUS cek fee — fee mungkin sudah naik
            if success_count == 0 and attempt == 1:
            ...
            if last is None:
                return True, None
            ok = last.fee_cc <= _max_fee_cc
            if not ok:
                log.info("Fee gate block: %.6f CC > limit %.6f CC (attempt %d)", last.fee_cc, _max_fee_cc, attempt)
            return ok, last.fee_cc
...
def _collect_progress_stats(
    progress_entries: list[tuple[str, list[dict]]],
    tx_dist: dict,
    all_fees: list,
) -> tuple[int, int]:
    """Helper: kumpulkan stats dari list (acc_name, tx_log). Return (success, failed)."""
    total_success = 0
    total_failed  = 0
    for acc_name, tx_log in progress_entries:
        success_count = sum(1 for t in tx_log if t.get("success"))
        ...
        for t in tx_log:
            ...
            if fee is not None:
                try:
                    ...
                    pass
            total_success += 1 if t.get("success") else 0
            total_failed  += 0 if t.get("success") else 1
    return total_success, total_failed


async def _build_daily_summary(today: str) -> str:
    """
    Rangkuman harian — dikirim tiap jam 07.00 WIB (00.00 UTC).
    Mengambil data dari Railway 1 (lokal) DAN Railway 2-4 (remote via HTTP).
    """
    all_fees: list[float] = []
    tx_dist:  dict[int, list[str]] = {}
    total_success_tx = 0
    total_failed_tx  = 0
    total_accounts   = 0

    # Railway 1 — lokal
    r1_progress = ProgressStore.load_all_today(today)
    entries_r1 = [(name, prog.tx_log) for name, prog in r1_progress.items()]
    if entries_r1:
        s, f = _collect_progress_stats(entries_r1, tx_dist, all_fees)
        total_success_tx += s
        total_failed_tx  += f
        total_accounts   += len(entries_r1)

    # Railway 2-4 — remote
    executor_data = await fetch_all_executor_progress()
    for railway_num, prog_data in executor_data:
        if not prog_data or "accounts" not in prog_data:
            continue
        entries = [(ap["name"], ap.get("tx_log", [])) for ap in prog_data["accounts"]]
        if entries:
            s, f = _collect_progress_stats(entries, tx_dist, all_fees)
            total_success_tx += s
            total_failed_tx  += f
    ...
    if all_fees:
        # Filter outlier: fee > 10 CC kemungkinan data stale / error
        clean_fees = [f for f in all_fees if f <= 10.0]
        if clean_fees:
            avg_fee = sum(clean_fees) / len(clean_fees)
            lines.append("")
            lines.append("  Gas Fee (saat trigger):")
            lines.append(f"    {'Average':<12}:  {avg_fee:.6f} CC")
            lines.append(f"    {'Min':<12}:  {min(clean_fees):.6f} CC")
            lines.append(f"    {'Max':<12}:  {max(clean_fees):.6f} CC")
            lines.append(f"    {'Triggers':<12}:  {len(clean_fees)} data point")
    else:
        lines.append("\n  Tidak ada data fee.")
