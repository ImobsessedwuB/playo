...
class CantexOrchestrator:
    ...
    async def _on_fee_trigger_watcher(self, result: FeeResult) -> None:
        # Cek bot state SEBELUM push signal — jika bot stopped, executor juga tidak eksekusi
        ...
        if self._panel:
            await self._panel.send_to_admin(
                f"Fee trigger: {result.fee_cc:.6f} CC — sinyal dikirim ke executor"
            )
        async with self._execution_lock:
            today = datetime.now(timezone.utc).date().isoformat()
            accounts = CredentialStore.load_all()  # Railway 1 local accounts
...
def _build_summary(results: list[DayResult], fee_cc: float) -> str:
    today = datetime.now(timezone.utc).date().isoformat()
    lines = [f"Eksekusi selesai - {today}", f"Fee saat eksekusi: {fee_cc:.6f} CC", ""]
    ...
    for r in results:
        ...
        for tx in r.tx_results:
            ...
            lines.append(
                ...
                f" | fee={tx.fee_cc:.6f} CC | {status}{err}"
            )
        total_tx += r.total_tx
