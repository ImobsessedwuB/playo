...
class SignalServer:
    ...
    async def _handle_get_fee(self, request: web.Request) -> web.Response:
        ...
        if last is None or age > 130:   # lebih dari 2 poll cycles = stale
        ...
        try:
            ...
            for acct in accounts:
                ...
                result_accounts.append({
                    ...
                    "proxy": acct.proxy.split("@")[-1] if acct.proxy else None,
                })
...
class SignalClient:
    ...
    async def check_fee_ok(self) -> tuple[bool, float | None]:
        """
        Cek apakah fee saat ini masih <= limit di Railway #1.
        Return (ok, fee_cc).
        - ok=True  → fee masih oke, lanjut TX pair
        - ok=False → fee naik / stale, hentikan eksekusi sementara
        Dipanggil executor sebelum tiap TX pair (fee-gating).
        """
        if not self._watcher_url:
            return True, None   # mode lokal, anggap selalu ok
        try:
            ...
            log.debug("check_fee_ok error: %s", exc)
        return False, None

    async def check_fee_ok(self) -> tuple[bool, float | None]:
        """
        Cek apakah fee saat ini masih di bawah limit di Railway #1.
        Return (ok, fee_cc). ok=False berarti fee naik / stale / error.
        Digunakan executor untuk fee-gating antar TX pair.
        """
        if not self._watcher_url:
            return True, None   # no watcher = lokal, anggap ok
        try:
            url = self._watcher_url + "/fee"
            async with aiohttp.ClientSession() as s:
                async with s.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
