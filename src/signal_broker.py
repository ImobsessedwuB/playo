...
class ExecutorServer:
    ...
    async def _handle_accounts(self, request: web.Request) -> web.Response:
        """Kembalikan jumlah akun yang tersimpan di executor ini."""
...
async def fetch_executor_account_count(url: str) -> int:
    """Ambil jumlah akun dari satu executor Railway. Return 0 kalau gagal."""
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                url + "/accounts",
...
async def fetch_all_executor_account_counts() -> int:
    """Total akun di semua executor Railways (Railway 2-4)."""
