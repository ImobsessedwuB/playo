...
class DailyProgress:
    utc_date: str = ""
    success_tx: int = 0        # jumlah TX yang BERHASIL — penentu "done"
    completed_tx: int = 0      # total attempt (sukses + gagal) — untuk info/log
    tx_log: list[dict] = field(default_factory=list)

    def to_dict(self) -> dict:
        return {
            "utc_date": self.utc_date,
            "success_tx": self.success_tx,
    ...
    def from_dict(d: dict) -> "DailyProgress":
        tx_log = d.get("tx_log", [])
        # Backward compat: hitung success_tx dari tx_log kalau field belum ada
        saved_success = d.get("success_tx")
        if saved_success is None:
            saved_success = sum(1 for t in tx_log if t.get("success"))
        return DailyProgress(
            utc_date=d.get("utc_date", ""),
            success_tx=int(saved_success),
            completed_tx=int(d.get("completed_tx", len(tx_log))),
            tx_log=tx_log,
        )
