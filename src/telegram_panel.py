...
class TelegramPanel:
    ...
    async def _send_main_menu(self, reply_fn) -> None:
        ...
        has_proxy = any(a.proxy for a in local_accounts)
        ...
        await reply_fn(
            text,
            reply_markup=_main_menu_keyboard(has_any_accounts, has_proxy, state.running),
        )
    ...
    async def _handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # ── Start Bot ──
            ...
            await query.message.reply_text(
                ...
                f"{_lux_field('Accounts', str(total_accounts))}\n"
    ...
    async def _recv_creds(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if state["current"] > total:
            # Semua credentials terkumpul — tanya proxy dulu sebelum kirim
            collected   = state["collected"]
            railway_num = state.get("railway", 1)
            state["action"]      = "creds_proxy"
            state["proxy_index"] = 0
            acct = collected[0]
            await update.message.reply_text(
                f"{_lux_title('Set Proxy')}\n\n"
                f"{_lux_field('Railway', str(railway_num))}\n"
                f"{_lux_field('Account', acct.name)} (1/{len(collected)})\n\n"
                "  Masukkan proxy untuk akun ini.\n"
                "  Format: username:password@ip:port\n"
                "  Ketik skip untuk tidak pakai proxy.\n\n"
                "  /cancel to abort"
            )
            return STATE_AWAIT_PROXY
        else:
    ...
    async def _recv_proxy(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # ── Mode 1: set proxy untuk akun lokal Railway 1 ──────────────────
        if action == "proxy":
        ...
        # ── Mode 2: set proxy saat input credentials (sebelum kirim ke Railway) ──
        if action == "creds_proxy":
            text      = update.message.text.strip()
            collected = state["collected"]
            pidx      = state["proxy_index"]
            acct      = collected[pidx]

            if text.lower() != "skip":
                if "@" not in text or ":" not in text.split("@")[0]:
                    await update.message.reply_text(
                        "Format salah. Gunakan: username:password@ip:port\n"
                        "Atau ketik skip untuk tidak pakai proxy."
                    )
                    return STATE_AWAIT_PROXY
                acct.proxy = text

            next_pidx = pidx + 1
            if next_pidx < len(collected):
                # Masih ada akun lain
                state["proxy_index"] = next_pidx
                nxt = collected[next_pidx]
                railway_num = state.get("railway", 1)
                await update.message.reply_text(
                    f"{_lux_title('Set Proxy')}\n\n"
                    f"{_lux_field('Railway', str(railway_num))}\n"
                    f"{_lux_field('Account', nxt.name)} ({next_pidx + 1}/{len(collected)})\n\n"
                    "  Masukkan proxy untuk akun ini.\n"
                    "  Format: username:password@ip:port\n"
                    "  Ketik skip untuk tidak pakai proxy.\n\n"
                    "  /cancel to abort"
                ...
                # Semua proxy terkumpul — kirim ke Railway
                railway_num = state.get("railway", 1)
                if railway_num == 1:
                    for c in collected:
                ...
                proxy_summary = sum(1 for c in collected if c.proxy)
                await update.message.reply_text(
                    f"{_lux_title('Credentials Saved')}\n\n"
                    f"{_lux_field('Result', result_str)}\n"
                    f"{_lux_field('Accounts', str(len(collected)))}\n"
                    f"{_lux_field('Proxy set', f'{proxy_summary}/{len(collected)}')}\n"
