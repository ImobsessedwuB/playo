...
def _main_menu_keyboard(has_accounts: bool, bot_running: bool) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton("Set Proxy", callback_data="set_proxy")])
...
class TelegramPanel:
    ...
    async def _show_menu(self, send_fn=None, edit_msg=None) -> None:
        ...
        markup = _main_menu_keyboard(total_accounts > 0, state.running)
    ...
    async def _handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # ── Set Proxy ──
            ...
            acct = accounts[0]
            proxy_list = acct.get_proxy_list()
            cur = ""
            if proxy_list:
                cur = "\n" + _field("Proxies", f"{len(proxy_list)} configured")
            await query.edit_message_text(
                f"SET PROXY — Account 1/{len(accounts)}\n\n"
                f"{_field('Account', acct.name)}{cur}\n\n"
                "  Input satu atau beberapa proxy (satu per baris).\n"
                "  Jika >1 proxy, akan dirotasi otomatis saat error.\n\n"
    ...
    async def _recv_creds(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if state["current"] > total:
            # Semua credentials terkumpul — minta semua proxy sekaligus
            collected   = state["collected"]
            railway_num = state.get("railway", 1)
            state["action"] = "creds_proxy_bulk"

            await update.effective_chat.send_message(
                f"PROXY CONFIGURATION\n\n"
                f"{_field('Railway', str(railway_num))}\n"
                f"{_field('Accounts', str(len(collected)))}\n\n"
                f"  Paste {len(collected)} proxy, satu per baris.\n"
                "  Format: user:pass@ip:port\n\n"
                "  Atau ketik 'skip' untuk semua akun tanpa proxy.\n\n"
                "  /cancel to abort",
                reply_markup=_KB_BACK,
            )
            return STATE_AWAIT_PROXY
    ...
    async def _recv_proxy(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # ── Mode 1: set proxy untuk Railway 1 (dari menu Set Proxy) ──
        # Mendukung multi-proxy: input beberapa baris, semua disimpan untuk rotasi
        if action == "proxy":
            ...
            if text.lower() != "skip":
                lines = [l.strip() for l in text.splitlines() if l.strip() and l.strip().lower() != "skip"]
                for ln in lines:
                    if "@" not in ln:
                        await update.message.reply_text(
                            f"Format salah: {ln[:30]}\n"
                            "  Required: user:pass@ip:port\n"
                            "  Bisa input beberapa proxy (satu per baris) untuk rotasi.\n"
                            "  Type 'skip' to leave unchanged."
                        )
                        return STATE_AWAIT_PROXY
                if lines:
            ...
            if next_idx >= len(accounts):
                ...
                cur = f"\n{_field('Current', nxt.proxy)}" if nxt.proxy else ""
                await update.effective_chat.send_message(
                    f"SET PROXY — Account {next_idx+1}/{len(accounts)}\n\n"
                    f"{_field('Account', nxt.name)}{cur}\n\n"
                    "  Format: user:pass@ip:port\n"
                    "  Type 'skip' to leave unchanged.\n\n"
                    "  /cancel to abort",
                    reply_markup=_KB_BACK,
        ...
        # ── Mode 2: bulk proxy setelah input credentials ──
        if action == "creds_proxy_bulk":
            text      = update.message.text.strip()
            collected = state["collected"]
            railway_num = state.get("railway", 1)
            ...
            if text.lower() != "skip":
                ...
                if len(lines) != len(collected):
                    await update.message.reply_text(
                        f"Jumlah proxy tidak sesuai.\n"
                        f"  Dibutuhkan: {len(collected)} baris\n"
                        f"  Diterima  : {len(lines)} baris\n\n"
                        "  Paste ulang semua proxy (satu per baris),\n"
                        "  atau ketik 'skip' untuk semua tanpa proxy.\n\n"
                        "  /cancel to abort"
                    )
                    return STATE_AWAIT_PROXY
                ...
                for i, line in enumerate(lines):
                    if line.lower() != "skip":
                        if "@" not in line:
                            await update.message.reply_text(
                                f"Baris {i+1}: format salah.\n"
                                "  Required: user:pass@ip:port\n"
                                "  Atau 'skip' untuk akun itu saja.\n\n"
                                "  /cancel to abort"
                            )
                            return STATE_AWAIT_PROXY
                        collected[i].proxy = line
                        if line not in collected[i].proxies:
                            collected[i].proxies = [line] + [p for p in collected[i].proxies if p != line]

            # Delete proxy message for security
            try:
                await update.message.delete()
            except Exception:
                pass

            # Simpan / kirim ke Railway
            if railway_num == 1:
                for c in collected:
                ...
                if res.get("ok"):
                    ...
                    result_str = f"FAILED sending to Railway {railway_num}: {res.get('error', 'unknown')}"
                    log.warning("Gagal kirim credentials ke Railway %d: %s", railway_num, res)

            self._pending_input.pop(uid, None)
            proxy_count = sum(1 for c in collected if c.proxy)
            await update.effective_chat.send_message(
                "CREDENTIALS SAVED\n\n"
                f"{_field('Result', result_str)}\n"
                f"{_field('Accounts', str(len(collected)))}\n"
                f"{_field('Proxy set', f'{proxy_count}/{len(collected)}')}\n\n"
                "  Use /menu to continue.",
                reply_markup=_KB_BACK,
            )
            if self._on_config_updated:
        ...
        for railway_num, prog_data in executor_progress:
            ...
            if prog_data and "accounts" in prog_data:
                ...
                for ap in r_accounts:
                    ...
                    proxy_str = ap.get("proxy") or "none"
                    lines.append(f"  {name:<18}  {completed}/6 tx   proxy: {proxy_str}")
            else:
                lines.append(f"  RAILWAY {railway_num} — unreachable")
