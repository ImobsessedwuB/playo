...
class TelegramPanel:
    ...
    async def start(self) -> None:
        log.info("Menginisialisasi Telegram bot...")
        try:
            self._app = (
                Application.builder()
                .token(self._token)
            ...
            # -------------------------------------------------------
            # FIX #1: Hapus webhook yang mungkin masih aktif.
            # Jika bot pernah dipakai dengan webhook, polling akan
            # di-ignore oleh Telegram sampai webhook dihapus.
            # -------------------------------------------------------
            try:
                await app.bot.delete_webhook(drop_pending_updates=True)
                log.info("Webhook dihapus / dipastikan tidak aktif")
            except Exception as exc:
                log.warning("Gagal delete_webhook (mungkin sudah bersih): %s", exc)
            ...
            conv_handler = ConversationHandler(
                ...
                fallbacks=[
                    ...
                    CommandHandler("start", self._cmd_start),  # FIX #2: izinkan re-entry /start
                ],
                per_chat=True,
                allow_reentry=True,
            )
            ...
            # FIX #3: hapus duplikat CommandHandler "menu" di luar ConversationHandler
            # agar tidak konflik dengan state handler
            ...
            await app.updater.start_polling(
                ...
                allowed_updates=Update.ALL_TYPES,  # FIX #4: pastikan semua update diterima
            )

            # -------------------------------------------------------
            # FIX #5: Verifikasi koneksi bot dengan get_me()
            # Ini akan langsung error jika token salah atau jaringan
            # bermasalah, sehingga muncul di log Railway.
            # -------------------------------------------------------
            me = await app.bot.get_me()
            log.info("Telegram bot TERHUBUNG: @%s (id=%s)", me.username, me.id)

        except Exception as exc:
            log.error(
                "GAGAL START TELEGRAM BOT: %s\n"
                "Pastikan TELEGRAM_BOT_TOKEN benar dan tidak ada webhook aktif.",
                exc,
                exc_info=True,
            )
            raise  # re-raise agar main() tahu bot gagal start
